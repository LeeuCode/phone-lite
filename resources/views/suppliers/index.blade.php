...
@section('content')
	<a href="{{ route('user.suppliers.create') }}" class="btn btn-info mt-3 mr-3" >
							...
							<tr>
								...
								<td>
									<form action="{{ route('user.destroy', ['id' => $user->id]) }}" onsubmit="return confirm('{{ __('هل أنت متأكد من حذف العنصر؟!') }}')" method="post">
										@csrf
										<a href="{{ route('user.supplier.edit', ['id' => $user->id]) }}" class="btn btn-outline-info" >
		  ...
		  <div class="modal-dialog">
		    <div class="modal-content">
		      <div class="modal-header">
		        <h4 class="modal-title">{{ __('البحث عن الموردين') }}</h4>
