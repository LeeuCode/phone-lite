...
@section('content')
  ...
  <div class="col-md-4 mt-3">
    <div class="card card-info">
      <div class="card-header">
        <h3 class="card-title">{{ __('أضف موديل جديد') }}</h3>
      </div>
			<!-- form start -->
      <div class="card-body">
  ...
  <div class="col-md-8 mt-3">
    <div class="card card-secondary">
      <div class="card-header">
        <h3 class="card-title">{{ __('الموديلات') }}</h3>
      </div>
      <div class="card-body p-0">
        <table class="table">
          <thead>
            <tr>
              <th>{{ __('اسم الموديل') }}</th>
              <th>{{ __('التحكم') }}</th>
            </tr>
          </thead>
          <tbody>
								...
								@foreach ($categories as $category)
									<tr>
										...
										<td class="py-0 align-middle">
											...
											<form class="p-0 m-0" action="{{ route('category.status', ['id' => $category->id]) }}" onsubmit="return confirm('{{ __('هل انت متاكد من حذف الموديل !') }}')" method="post">
												@csrf
						        ...
						        <div class="modal-dialog">
						          <div class="modal-content">
						            ...
						            <div class="modal-body">
														<input type="hidden" name="type" value="model">
									          <div class="form-group">
									  						<label for="barcode" class="control-label">{{ __('اسم الموديل') }}</label>
									              <input type="text" name="title" class="form-control" id="barcode" placeholder="{{ __('اكتب اسم الموديل هنا') }}">
											      </div>
						            </div>
							...
							@else
								<tr>
									<td colspan="2"class="text-center" >
										{{ __('لم يتم لعثور علي اي موديل حتي الان') }}
									</td>
								</tr>
							@endif
						@else
