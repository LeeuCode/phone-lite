...
@section('content')
    <form id="invoice-save" class="w-100 row" action="{{ route('invoice.update', ['id' => $id]) }}" method="post">
                            ...
                            <tr id="{{ $item->item->barcode }}" class="items">
                                <td class="text-center d-none">
                                    <input type="hidden" name="item[id][]" value="{{ $item->item->id }}"
                                        class="form-control item_id w-100 border-0" readonly>
                                </td>
                                <td style="width:15%" class="text-center barcode">{{ $item->item->barcode }}</td>
                                <td style="width:25%" class="text-center">
                                    <input type="text" class="form-control p-0 title border-0 text-center"
                                        value="{{ $item->item->title }}" readonly>
                                </td>
                                <td style="width:15%" class="text-center">
                                    @php
                                        if ($invoice->invoice_type == 'sale') {
                                            $storeBlance = $item->store_balance + $item->quantity;
                                        } else {
                                            $storeBlance = $item->store_balance - $item->quantity;
                                        }
                                    @endphp
                                    <input type="number" name="item[store_balance][]" value="{{ $item->store_balance }}"
                                        class="form-control store_balance w-100 border-0" data-balance="{{ $storeBlance }}"
                                        readonly>
                                </td>
                                <td class="text-center purchasing" {!! !$show ? 'style="display: none;"' : '' !!}>
                                    <input type="number" name="item[purchasing_price][]"
                                        value="{{ $item->purchasing_price }}"
                                        class="form-control purchasing_price w-100 {!! getVal($inv, 'invoice_type') != 'purchase' && !$show ? 'border-0' : '' !!}"
                                        {!! getVal($inv, 'invoice_type') != 'purchase' && !$show ? 'readonly' : '' !!}>
                                </td>
                                <td class="text-center">
                                    <input type="number" name="item[selling_price][]" value="{{ $item->selling_price }}"
                                        class="form-control price w-100 border-0" readonly>
                                </td>
                                <td>
                                    <input type="number" name="item[qt][]" min="1" value="{{ $item->quantity }}"
                                        class="form-control form-control-sm text-center qt" placeholder="00">
                                </td>
                                <td class="text-center col-md-1">
                                    <input type="number" name="item[item_total][]" value="{{ $item->total }}"
                                        class="form-control w-100 item-total border-0 p-0 text-center" readonly>
                                </td>
                                <td>
