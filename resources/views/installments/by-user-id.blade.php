...
@section('content')
  <h3 class="mt-3 col-md-4" >{{ $customer->title }}</h3>
    ...
    <ul class="nav">
      <li class="nav-item">
        <a class="p-1 f-12 text-danger" href="{{ route('installment.user', ['id' => request('id')]) }}">
          <small>{{ __('أقساط جاريه') }} ({{ DB::table('installments')->where('remaining_installments','!=', 0)->where('customer_id', request('id'))->count() }})</small>
        </a>
      </li>
      |
      <li class="nav-item text-danger">
        <a class="p-1" href="{{ route('installment.paids') }}">
          <small>{{ __('أقساط مسدده') }} ({{ DB::table('installments')->where('remaining_installments', 0)->where('customer_id', request('id'))->count() }})</small>
        </a>
      </li>
    </ul>
  </div>
