					...
					<tbody>
						@foreach ($users as $user)
							<tr>
								...
								<td>
									<form action="{{ route('item.status', ['id' => $user->id]) }}" onsubmit="return confirm('{{ __('هل أنت متأكد من حذف العنصر؟!') }}')" method="post">
										@csrf
										<a href="{{ route('users.employee.edit', ['id' => $user->id]) }}" class="btn btn-outline-info" >
