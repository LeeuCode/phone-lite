    ...
    <!-- form start -->
      <div class="card-body">
        <div class="form-group">
          ...
          <input type="number" name="total" value="{{ getVal($inv, 'total') }}" class="form-control form-control-sm text-center totalItems" id="total" placeholder="00" readonly>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="number" name="total_discount" class="form-control form-control-sm text-center total_discount" id="total_discount" value="{{ getVal($inv, 'total_discount') }}" placeholder="00" readonly>
        </div>

        <div class="form-group">
          <label for="tax_rate" class="control-label small">{{ __('نسبة الضريبه') }}</label>
          <input type="text" name="tax_rate" class="form-control form-control-sm text-center" value="{{ getVal($inv, 'tax_rate') }}" id="tax_rate" placeholder="00" {!! (isset($view)) ? 'readonly' : '' !!}>
        </div>
        <div class="form-group">
          <label for="tax_value" class="control-label small">{{ __('قيمة الضريبه') }}</label>
          <input type="text" name="tax_value" class="form-control form-control-sm text-center" value="{{ getVal($inv, 'tax_value') }}" id="tax_value" readonly placeholder="00" {!! (isset($view)) ? 'readonly' : '' !!}>
        </div>
        <div class="form-group">
          <label for="total_tax" class="control-label small">{{ __('إجمالي بعد الضريبه') }}</label>
          <input type="text" name="total_tax" class="form-control form-control-sm text-center" id="total_tax" readonly value="{{ getVal($inv, 'total_tax') }}" placeholder="00" {!! (isset($view)) ? 'readonly' : '' !!}>
        </div>
